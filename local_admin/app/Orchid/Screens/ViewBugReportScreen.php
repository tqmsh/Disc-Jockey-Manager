...
class ViewBugReportScreen extends Screen
{
    ...
    {
        return [
            'user_bug_reports' => BugReport::filter(request(['severity', 'status']))->where('reporter_user_id', Auth::user()->id)->paginate(request()->query('pagesize', 10))->sortByDesc('created_at')
        ];
    }
