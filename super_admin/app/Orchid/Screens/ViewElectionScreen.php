...
class ViewElectionScreen extends Screen
{
    ...
    public function query(Events $event): iterable
    {
        ...
        $position = Position::where('election_id', $election->id)->paginate(min(request()->query('pagesize', 10), 100));
        $candidate = Candidate::where('election_id',$election->id)->paginate(min(request()->query('pagesize', 10), 100));
        return [
            'event' => $event,
            'election' => $election,
            'position' => $position,
