...
class ViewBidHistoryScreen extends Screen
{
    ...
    public function query(): iterable
    {
        ...
        return [
            'eventBids' => EventBids::filter(request(['region_id']))->where('user_id', Auth::user()->id)->orderBy('status')->paginate(min(request()->query('pagesize', 10), 100)),
            'studentBids' => StudentBids::filter(request(['region_id']))->where('user_id', Auth::user()->id)->orderBy('status')->paginate(min(request()->query('pagesize', 10), 100)),
            'limoBids' => LimoGroupBid::where('user_id', Auth::user()->id)->orderBy('status')->paginate(min(request()->query('pagesize', 10), 100)),
            'beautyBids' => BeautyGroupBid::where('user_id', Auth::user()->id)->orderBy('status')->paginate(min(request()->query('pagesize', 10), 100)),
