    ...
    public function query(): iterable
    {
        ...
        return [
            ...
            'students' => Student::whereIn('school_id', School::whereIn('region_id', $this->paidRegionIds)->pluck('id'))
                ->whereJsonContains('interested_vendor_categories', Auth::user()->vendor->category_id)
                ->paginate(request()->query('pagesize', 10)),
            'limo_groups' => (stripos(Auth::user()->vendor->category->name, 'limo') !== false) ? LimoGroup::whereIn('school_id', School::whereIn('region_id', $this->paidRegionIds)->pluck('id'))->paginate(request()->query('pagesize', 10)) : null,
            'beauty_groups' => (stripos(Auth::user()->vendor->category->name, 'salon') !== false) ? BeautyGroup::whereIn('school_id', School::whereIn('region_id', $this->paidRegionIds)->pluck('id'))->paginate(request()->query('pagesize', 10)) : null,
        ];
    }
