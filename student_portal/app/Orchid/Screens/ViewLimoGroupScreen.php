...
class ViewLimoGroupScreen extends Screen
{
    ...
    public function query(): iterable
    {
        ...
        return [
            ...
            'current_limo_group_members' => ($current_limo_group != null) ? $current_limo_group->members()->paginate(request()->query('pagesize', 10)) 
                                    : (($owned_limo_group != null) ? $owned_limo_group->members()->paginate(request()->query('pagesize', 10))
                                    : []),
            'limo_group_invitations' => LimoGroupMember::where('invitee_user_id', Auth::user()->id)->where('status', 0)->paginate(request()->query('pagesize', 10)),
            'limoBids' => LimoGroupBid::where('limo_group_id', ($owned_limo_group != null) ? $owned_limo_group->id : 0)->where('status', 0)->paginate(request()->query('pagesize', 10)),
            'default' => null,
        ];
    }
