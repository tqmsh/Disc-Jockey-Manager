...
class ViewChecklistItemScreen extends Screen
{
    ...
    public function query(Checklist $checklist): iterable
    {
        ...
        return [
            'checklist' => $checklist,
            'completed_checklist_items' => ChecklistUser::where('checklist_user_id', Auth::user()->id)->where('checklist_id', $checklist->id)->latest()->get(),
            'incomplete_checklist_items' => ChecklistItem::whereNotIn('id', ChecklistUser::where('checklist_user_id', Auth::user()->id)->where('checklist_id', $checklist->id)->pluck('checklist_item_id'))->latest()->get()
        ];
    }
